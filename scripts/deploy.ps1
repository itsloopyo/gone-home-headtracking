...
# Mono.Cecil is fetched once into tools/ and reused across deploys. The
...
$result = Invoke-DevDeployCecil `
    ...
    -Patcher {
        param($assemblyPath)
        # Gone Home uses a custom Mono.Cecil bootstrap patcher (compiled
        # from BootstrapPatcher.cs) rather than the shared screen-center
        # patcher in AssemblyPatching.psm1, because Unity 2017 Mono on
        # Gone Home needs a different injection point.
